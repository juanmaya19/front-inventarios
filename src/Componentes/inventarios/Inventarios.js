...
import { borrarInventarioPorID, crearInventario, editarInventarioPorID, obtenerInventario } from '../../services/InventarioService'
...
export default function Estados() {
  ...
  const listInventarios= async () => {
    ...
    try{
      setError(false)
      const { data } = await obtenerInventario(query)
      setInventarios(data)
      setLoading(false)
    }catch(e){
      console.log(e)
  ...
  const guardarInventario = async () => {
    ...
    setLoading(true)
    try{
      const res = await crearInventario(estado)
      console.log(res)
      setLoading(true)
      setEstado({nombre: ''})
      listInventarios()
  ...
  const editarInventario = async (e) => {
    ...
    try{
      setError(false)
      const resp = await editarInventarioPorID(estado._id, estado);
      console.log(resp)
      resetEstado()
      listInventarios()
    }catch(e){
  ...
  const setInventarioPorId = (e) => {
    ...
    const estadosFilter = inventario.filter(t => t._id == e.target.id);
